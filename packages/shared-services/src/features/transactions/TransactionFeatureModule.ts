...
export class TransactionFeatureModule implements IFeatureModule {
	...
	public readonly description =
		"Manages financial transactions, balances, and related operations";
	 ...
	 */
	async createTransaction(data: Omit<Transaction, "id">): Promise<void> {
		...
		return {
			...
			averageAmount:
				transactions.length > 0
					? Math.abs(totalAmount) / transactions.length
					: 0,
	...
	private async initializeUseCases(): Promise<void> {
		if (
			!this.listTransactionsRepository ||
			!this.createTransactionRepository ||
			!this.updateTransactionRepository ||
			!this.deleteTransactionRepository ||
