...
export class ServiceRegistry {
   ...
   */
  registerClass<T>(target: new (...args: any[]) => T, token?: string | symbol): IServiceContainer {
    const validation = validateInjectable(target);
    if (!validation.isValid) {
      throw new Error(`Cannot register ${target.name}: ${validation.errors.join(', ')}`);
    }
    ...
    return this.container.registerFactory(serviceToken, (container) => {
   ...
   */
  registerClasses(classes: Array<new (...args: any[]) => any>): IServiceContainer {
   ...
   */
  private createInstance<T>(target: new (...args: any[]) => T, container: IServiceContainer): T {
    ...
    paramTypes.forEach((paramType, index) => {
      ...
      if (!this.container.isRegistered(token)) {
        try {
          ...
          if (paramType && paramType !== Object && this.hasInjectableMetadata(paramType)) {
   ...
   */
  private extractClassesFromModule(module: any): Array<new (...args: any[]) => any> {
    const classes: Array<new (...args: any[]) => any> = [];

    for (const key in module) {
      const exported = module[key];

      if (this.isClassConstructor(exported) && this.hasInjectableMetadata(exported)) {
