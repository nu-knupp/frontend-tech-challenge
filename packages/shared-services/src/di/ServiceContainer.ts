import { IServiceContainer, ServiceDescriptor, ServiceLifetime, ServiceRegistrationOptions } from './IServiceContainer';
...
export class ServiceContainer implements IServiceContainer {
  ...
  register<T>(
    ...
    if (existing && !options.replace) {
      throw new Error(`Service "${String(token)}" is already registered. Use replace: true to overwrite.`);
    }
  ...
  isRegistered(token: string | symbol): boolean {
    const key = this.getKey(token);
    return this.services.has(key) || (this.parent?.isRegistered(token) ?? false);
  }
    ...
    for (const [key, instance] of this.scoped) {
      if (instance && typeof instance.dispose === 'function') {
        try {
          ...
          console.error(`Error disposing scoped service "${String(key)}":`, error);
        }
      }
    }
  ...
  private createInstance<T>(descriptor: ServiceDescriptor): T {
    ...
    if (descriptor.lifetime === ServiceLifetime.Singleton) {
      ...
      const instance = this.createNewInstance(descriptor);
      this.singletons.set(key, instance);
      return instance;
    }
    ...
    return this.createNewInstance(descriptor);
  }
  ...
  private createNewInstance<T>(descriptor: ServiceDescriptor): T {
    try {
      ...
      if (descriptor.implementation && typeof descriptor.implementation === 'function') {
        const dependencies = this.resolveDependencies(descriptor.dependencies || []);
        return new descriptor.implementation(...dependencies);
      }

      throw new Error(`Invalid service configuration for "${String(descriptor.token)}"`);
    } catch (error) {
      throw new Error(`Failed to create instance of service "${String(descriptor.token)}": ${error}`);
    }
  }
