...
// Extend Reflect type to include metadata methods
declare global {
	namespace Reflect {
		function getMetadata(metadataKey: string | symbol, target: object): any;
...
export function Injectable(options?: {
	...
	return function (target: any) {
		const existingMetadata =
			Reflect.getMetadata(INJECTABLE_METADATA_KEY, target) || {};
...
export function InjectMany(token: string | symbol): ParameterDecorator {
	return function (
		...
		const existingTokens =
			Reflect.getMetadata(INJECT_MANY_METADATA_KEY, target) || [];
		existingTokens[parameterIndex] = token;
...
export function AutoInject(token: string | symbol): PropertyDecorator {
	return function (target: any, propertyKey: string | symbol) {
		const existingProperties =
			Reflect.getMetadata("autoinject_properties", target) || {};
 ...
 */
export function getAutoInjectProperties(
	target: any,
): Record<string | symbol, string | symbol> {
...
export function validateInjectable(target: any): {
	...
	if (paramTypes.length !== paramTokens.length && paramTokens.length > 0) {
		errors.push(
			`Parameter count mismatch: ${paramTypes.length} types vs ${paramTokens.length} tokens`,
		);
	}
	...
	if (paramTypes.some((type) => type === undefined || type === null)) {
		errors.push(
			'Missing parameter type information. Ensure "emitDecoratorMetadata": true in tsconfig.json',
		);
	}
